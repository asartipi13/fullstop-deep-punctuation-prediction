...
def to_tsv(lines, filename: str):
    ...
    with open(filename, 'w', encoding='utf-8') as output_fh:
        prev = None
        cnt_lines = 0
        for line in lines:
            cnt_lines += 1
...
def dev_test_train(text_file: str, outpath: str):
    ...
    with open(text_file, "r", encoding='utf-8') as textfile_fh:
        ...
        to_tsv(train_lines, f"{outpath}/fa/train/train.tsv")
        to_tsv(dev_lines, f"{outpath}/fa/dev/dev.tsv")
        to_tsv(test_lines, f"{outpath}/fa/test/test.tsv")
...
if __name__ == '__main__':
    ...
    zipfile = 'sepp_nlg_2021_train_dev_data_v5.zip'
